...
10. **Keep device portability** — Most code is shared across accelerators (CUDA, XPU, ROCm, MPS). Report CUDA-specific code (`torch.cuda.*`, `device="cuda"`, `CUDACodeCache`, hardcoded `cuda` in expected-string assertions, CUDA-only test gating) on device-generic paths unless it is behind an explicit device guard.
